...
def uploadimage():
    if request.method == "POST":
        ...
        save_path = os.path.join('uploadimages', temp_name)
        image.save(save_path)
        prediction = model_predict(save_path)
        return render_template('home.html', result=True, imagepath=url_for('uploaded_images', filename=temp_name), prediction=prediction)
    else:
        return redirect('/')
